...
foreach(scope model_memory_query execution_io_counters)
  string(JSON dynamic_shape GET "${report}" ${scope} draft_dynamic_shape)
  string(JSON om_gears GET "${report}" ${scope} draft_om_dynamic_gear_count)
  string(JSON shape_plans GET "${report}" ${scope} draft_dynamic_gear_count)
  if("$ENV{QWEN35_DFLASH_FAKE_DYNAMIC_SHAPE}" STREQUAL "1")
    if(NOT dynamic_shape OR NOT om_gears EQUAL 0 OR NOT shape_plans EQUAL 18)
      message(FATAL_ERROR "dynamic Shape report fabricated OM gears: ${report}")
    endif()
  elseif(dynamic_shape OR NOT om_gears EQUAL shape_plans)
    message(FATAL_ERROR "discrete gear report has inconsistent execution mode: ${report}")
  endif()
endforeach()
