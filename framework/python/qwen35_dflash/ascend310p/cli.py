...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Qwen3.5 quant W8A8 DFlash -> TorchAir AIR -> ATC OM -> "
            "AscendCL C++ framework"
        )
    )
    ...
    infer.add_argument("--max-draft-tokens", type=int, default=15)
    ...
    infer.set_defaults(handler=command_infer, model_asset_id=None)
    ...
    run_e2e.add_argument("--model-dir", type=Path, required=True)
    ...
    run_e2e.set_defaults(handler=command_run_e2e, model_asset_id=None)
    ...
    run_e2e_cpp.add_argument("--model-dir", type=Path, required=True)
