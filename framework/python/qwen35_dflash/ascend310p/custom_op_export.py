...
def _validate_npu_chunk_gated_delta_rule_meta(operation: Any) -> None:
    # Cover every physical gear: prefill64, verify16 (both passes), decode1.
    for rows in (1, 16, 64):
        query = torch.empty((1, rows, 32, 128), dtype=torch.float16, device="meta")
        key, value = torch.empty_like(query), torch.empty_like(query)
        gate = torch.empty((1, rows, 32), dtype=torch.float32, device="meta")
        beta = torch.empty((1, rows, 32), dtype=torch.float16, device="meta")
        effective = torch.empty((1,), dtype=torch.int16, device="meta")
        for initial in (None, torch.empty((1, 32, 128, 128),
                                          dtype=torch.float32, device="meta")):
            result = operation(query, key, value, gate, beta, effective,
                               1 if rows == 1 else 64, initial, True, True)
            if not isinstance(result, (tuple, list)) or len(result) != 2:
                raise RuntimeError("npu::npu_chunk_gated_delta_rule Meta must return two tensors")
            _expect_tensor(result[0], shape=(1, rows, 32, 128), dtype=torch.float16,
                           label="npu::npu_chunk_gated_delta_rule output[0]")
            _expect_tensor(result[1], shape=(1, 32, 128, 128), dtype=torch.float32,
...
def _register_framework_converter(
    ...
    if adapter.torch_op == ADN_RMS_NORM_TORCH_OP:
        if spec.ge_op_type not in {"RmsNorm", "AdnRmsNorm"}:
            raise RuntimeError("adn_rms_norm GE type must be RmsNorm or AdnRmsNorm")
        if call_mode != "registered-ir-positional":
            _require_ge_attrs(ge_api, ("Float",))

        def converter(input: Any, gamma: Any, epsilon: float = 1e-6,
                      meta_outputs: Any = None) -> Any:
            del meta_outputs
            if call_mode == "registered-ir-positional":
                return emit_positional(input, gamma, epsilon)
            session.converter_calls += 1
            input_name = "self" if spec.ge_op_type == "AdnRmsNorm" else "x"
            result = custom_op(
                spec.ge_op_type,
                inputs={input_name: input, "gamma": gamma},
                outputs=["y", "rstd"],
                attrs={"epsilon": ge_api.attr.Float(epsilon)},
            )
            if not isinstance(result, (tuple, list)) or len(result) != 2:
                raise RuntimeError(f"{spec.ge_op_type} GE IR must return y and rstd")
            return result
