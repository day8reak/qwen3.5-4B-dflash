...
[静态四图默认部署指南](docs/STATIC_SPLIT_OM_DEFAULT.md) 和
...
v48 编译器默认强制保留原图 dtype，GDR-MTP 路径不变。只更新 Python 或 C++ 不会改变旧
OM：需要从原 AIR 重新编译四个 OM；步骤及生效检查见
[OM 编译精度保护](docs/OM_COMPILER_PRECISION.md)。
