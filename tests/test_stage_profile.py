...
class FakeAdapter:
    ...
    def __init__(self, collector, events, *, anchor=10, fail_verify=False, reject=False,
        ...
        self.pending = None
        self.requests = 0
        self.gdr_verify_calls = self.gdr_commit_calls = 0
...
def test_only_one_stage_is_collected_after_fresh_warmup(setup_profiler, stage):
    ...
    assert result["captured_gdr_layer_calls"] == {
        "verify": 2 if stage in {"verify", "draft-verify"} else 0,
        "commit": 0,
    }
    if stage == "prefill":
...
def test_zero_acceptance_commit_still_requires_second_chunk_gdr(setup_profiler):
...
@pytest.mark.parametrize("stage", ["verify", "accept-commit", "draft-verify", "decode-round", "all"])
@pytest.mark.parametrize("accepted", [0, 1, 3])
def test_chunk_bridge_verify_and_commit_use_separate_capture_windows(setup_profiler, stage, accepted):
    ...
    class ObservedModel(FakeHIAIModel):
        def forward(self, **kwargs):
            events.append(("gdr-verify" if kwargs.get("dflash_chunk_verify") else "gdr-prefill",
                           collector.active))
            return super().forward(**kwargs)

        def commit_dflash_chunk_state(self, rows):
            events.append(("gdr-commit", collector.active, rows))
            return super().commit_dflash_chunk_state(rows)

    model = ObservedModel().eval()
    bridge = InternalDFlashTarget(
        FakeWrapper(model).eval(), device=torch.device("cpu"), dtype=torch.float16,
        kv_cache_max_len=64, rollback_enabled=True,
    ).eval()
    adapter = FakeAdapter(collector, events)
    adapter.target = bridge
    adapter.begin_rollback = lambda ids: bridge.begin_rollback(ids)["logits"]
    adapter._validated_output = lambda output, **kwargs: (output["logits"], output["dflash_features"])
    # The fixture Target chooses token 0. Exercise reject/partial/all accept.
    proposals = [0] * accepted + [11] * (3 - accepted)
    adapter.propose_rollback = lambda ids, limit: torch.tensor([proposals[:limit]])
    adapter.verify_rollback = lambda ids: bridge.verify_rollback(ids)["logits"]
    adapter.commit_rollback = bridge.commit_rollback
    adapter.abort_rollback = bridge.abort_rollback
    profile = profile_all_stages if stage == "all" else profile_one_stage
    kwargs = {} if stage == "all" else {"stage": stage}
    with profiler:
        report = profile(
            adapter, [1, 2], block_size=4, eos_token_ids=[], warmup=1,
            profiler=profiler, **kwargs,
        )
    reports = report["captures"] if stage == "all" else [report]
    start_indices = [i for i, e in enumerate(events) if e == ("start",)]
    stop_indices = [i for i, e in enumerate(events) if e == ("stop",)]
    assert len(start_indices) == len(stop_indices) == len(reports)
    for capture, start, stop in zip(reports, start_indices, stop_indices):
        selected = capture["profile_stage"]
        expected_verify = int(selected in {"verify", "draft-verify", "decode-round"})
        expected_commit = int(selected in {"accept-commit", "decode-round"})
        assert capture["captured_gdr_layer_calls"] == {
            "verify": expected_verify, "commit": expected_commit,
        }
        window = events[start + 1:stop]
        assert sum(e[0] == "gdr-verify" for e in window) == expected_verify
        commits = [e for e in window if e[0] == "gdr-commit"]
        assert commits == ([("gdr-commit", True, accepted + 1)] if expected_commit else [])
        if selected != "prefill":
            assert all(e[0] != "gdr-prefill" for e in window)
    assert bridge.dflash_rollback_audit["persistent_cursor"] == 2 + accepted + 1


def test_missing_second_gdr_pass_fails_capture_validation(setup_profiler):
    events, collector, profiler = setup_profiler("accept-commit")
    adapter = FakeAdapter(collector, events, reject=True)
    adapter.commit_rollback = lambda accepted: None
    with profiler, pytest.raises(RuntimeError, match="layer calls do not match"):
        profile_one_stage(
