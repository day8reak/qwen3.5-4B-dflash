...
| Target 实现 | Transformers/PyTorch | 同一代码放到 CUDA | HIAI NPU 实现；`quant` 分支可装配既有 `QLinear` |
...
`quant` 分支没有复制一套新的 Scheduler 或 Draft。它只在 NPU Target 装配阶段增加量化入口：

```text
Linear 量化权重 ── quantizer ──> Target nn.Linear → QLinear(W8A8)
embedding 权重 ─┐
embedding scale ├─ input provider ──> 第 0 层 FP16 hidden
input_ids ──────┘
```

- Target 的约定 Linear 使用动态 W8A8 `QLinear`；attention、GDN、cache 和 feature collector
  继续使用同一 NPU Target 路径。
- Draft 仍是官方 6 层 FP16 checkpoint，Draft backend 和 sequential verifier 不变。
- 三条数据路径不能合并成一个含义不明的参数；部署格式由两个 callback 解释，DFlash 不猜布局。
- ordinary 量化 Target 是权威基线，量化 DFlash 仍必须达到 token/EOS/stop reason 零差异。
- 不传量化参数时，`quant` 分支仍走原 FP16 Target 路线。

第一次运行不要从完整 DFlash 开始。准确顺序是普通量化 Target → Target-only preflight →
增量 Target 与 fresh full-prefix parity → 完整 DFlash。可复制命令见
[量化版运行与排错指南](DFLASH_V1_QUANT_RUNBOOK.md#0-最快跑起来按这五步)，数值和装配边界见
[NPU Quant Target 设计](DFLASH_V1_NPU_QUANT_DESIGN.md)。
...
7. 按设备选择运行文档：
   ...
   - `quant` 分支：[量化版运行与排错](DFLASH_V1_QUANT_RUNBOOK.md)
...
| `target_quant.py` | `quant` 分支的量化 callback、三路径和 QLinear 装配合同 |
| `preflight_target_quant.py` | 不读取 Draft 的量化 Target-only NPU 预检 |
| `w8a8_emulation.py` | CPU/CUDA correctness-only W8A8 Linear 公式仿真 |
