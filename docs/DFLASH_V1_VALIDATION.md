...
量化 Target 不能只复用上面的最终 token 比较。推荐按以下顺序执行，前一步失败就停止：

1. **普通量化 Target**：用原推理入口、相同 prompt 和三条量化数据路径正常生成；它是权威基线。
2. **Target-only preflight**：不读取 Draft，检查 quantizer、input-provider、完整 QLinear 拓扑、
   feature 零影响和有界 full-prefix 重复性。
3. **同 activation 单层对照**：捕获真实 QLinear 的输入/输出，用同一份 `W_q/scale` 运行 CPU
   W8A8 公式；避免 ordinary/DFlash 共用同一个错误 kernel 时产生假 PASS。
4. **增量与 full-prefix parity**：普通量化 prefill/decode 与 DFlash fresh full-prefix 的 Target
   Top-1 必须一致；feature 数值差异要定位到首个层。
5. **完整量化 DFlash**：至少真实执行一次 feature、Draft proposal 和 Target verify，并保持
   token/EOS/stop reason 与第 1 步完全一致。

量化报告里的 `PASS_ASSEMBLY_CONTRACT_NO_NUMERICAL_CLAIM` 只说明转换拓扑、shape、buffer、路径和
callback 调用合规，不代表真实 NPU 数值、整网正确性、接受率或性能已经通过。完整命令和报告
断言见 [量化版运行与排错指南](DFLASH_V1_QUANT_RUNBOOK.md)。
