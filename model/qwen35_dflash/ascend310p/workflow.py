...
def _run_declared_target_preflight() -> Path:
    ...
    if not preflight_value or not model_root_value:
        raise RuntimeError(
            "run-e2e requires an activated workspace target environment with "
            "AI_TARGET_PREFLIGHT and AI_MODEL_ROOT"
        )
    ...
    if not preflight.is_file() or not os.access(preflight, os.X_OK):
        raise RuntimeError(f"declared target preflight is not executable: {preflight}")
    command = [
        str(preflight),
        str(model_root),
    ...
    if result.returncode != 0:
        raise RuntimeError(
            "declared Ascend 310P preflight failed before checkpoint load: "
...
def load_tokenizer(
    *,
    ...
    """Load the locked tokenizer and return its auditable source identity."""
    ...
    if model_dir is not None:
        ...
        resource = resolve_locked_data(model_asset_id or "qwen3.5-4b")
        for item in resource.manifest.get("files", []):
            relative_path = str(item["path"])
            if not relative_path.startswith("model.safetensors"):
                resource.file(relative_path, verify=True)
        source_dir = resource.root
    ...
    source = {
        "path": str(source_dir),
        "asset_id": None if resource is None else resource.asset_id,
        "manifest_sha256": None if resource is None else resource.manifest_sha256,
    }
